...
import { EXPENSE_CATEGORIES, type ExpenseEntry } from "@/types/domain";
import { Button } from "@/components/ui/Button";
import { Card, CardContent, Input, Select, Skeleton } from "@/components/ui/primitives";
import { formatCurrency } from "@/lib/utils";

export function ExpensesTab({ monthId }: { monthId: string }) {
  const queryClient = useQueryClient();
  const [showForm, setShowForm] = useState(false);
  const [date, setDate] = useState(new Date().toISOString().slice(0, 10));
  const [category, setCategory] = useState<(typeof EXPENSE_CATEGORIES)[number]>("Bike Wash");
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get<{ data: ExpenseEntry[]; total: number; categoryTotals: Record<string, number> }>(`/months/${monthId}/expenses`),
  });
  ...
  const create = useMutation({
    mutationFn: () =>
      api.post(`/months/${monthId}/expenses`, {
        date,
        category,
        customCategoryLabel: category === "Custom" ? customLabel : null,
    ...
    onError: (err) => toast.error(err instanceof Error ? err.message : "Could not add expense"),
  });

  const remove = useMutation({
    mutationFn: (id: string) => api.delete(`/months/${monthId}/expenses/${id}`),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => setShowForm((s) => !s)}><Plus className="h-3.5 w-3.5" /> Add expense</Button>
      </div>
        ...
        <Card>
          <CardContent className="pt-5">
            <form onSubmit={handleSubmit} className="grid sm:grid-cols-4 gap-3 items-end">
              ...
              <div className={category === "Custom" ? "sm:col-span-4" : "sm:col-span-1"}>
                <label className="text-xs font-medium text-ink-muted mb-1.5 block">Note (optional)</label>
      ...
      {isLoading ? (
        ...
        <p className="text-sm text-ink-faint py-8 text-center">No expenses for this month yet</p>
      ) : (
          ...
          {data?.data.map((entry) => (
            <div key={entry.id} className="flex items-center justify-between px-4 py-3 bg-base-panel/40">
              ...
              <div className="flex items-center gap-3">
                <p className="tabular text-sm font-medium text-danger">{formatCurrency(entry.amount)}</p>
